...
class timer {
  ...
  void check() {
    elapsed = 3600 * hour() + 60 * minute() + second() + millis() * .0001 - start;
    if (elapsed > duration) {
      active = false;
    }
  }
...
void display() {
  ...
  printText(str(player.sprite), 50, 300, 10, orbitron, 144, #FFFFFF, LEFT);
  ...
  printText(str(player.boltPosition) + "/" + str(player.currentWeapon), 50, 150, 10, orbitron, 144, #FFFFFF, LEFT);
}